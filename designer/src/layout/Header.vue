<template>
  <div class="c-header">
    ...
    <div class="c-right">
      <div class="c-action" v-if="showToolbarMenu(HeaderToolbarMenu.New)">
        <a-button type="link" @click="newPkgHandle()">
          <icon type="new" />
          新建
        </a-button>
      </div>
      <div class="c-action" v-if="showToolbarMenu(HeaderToolbarMenu.Developer)">
      ...
      <div
        class="c-action"
        v-if="showToolbarMenu(HeaderToolbarMenu.Open, HeaderToolbarMenu.Recent)"
      >
          ...
          <template #overlay>
            <a-menu>
              <a-menu-item
                @click="loadPkg()"
                key="2"
                v-if="showToolbarMenu(HeaderToolbarMenu.Open)"
              >
                <icon type="open" />
                浏览
              </a-menu-item>
              <a-sub-menu
                v-if="
                  ...
                  showToolbarMenu(HeaderToolbarMenu.Recent)
                "
                key="recent"
              >
                <template #title>
      ...
      <div
        class="c-action"
        v-if="showToolbarMenu(HeaderToolbarMenu.Save, HeaderToolbarMenu.SaveAs)"
      >
        <a-dropdown @click="onSave()">
          ...
          <template #overlay>
            <a-menu>
              <a-menu-item
                v-if="showToolbarMenu(HeaderToolbarMenu.Save)"
                @click="onSave()"
                key="1"
              >
                <icon type="save" />
              ...
              <a-menu-item
                v-if="showToolbarMenu(HeaderToolbarMenu.SaveAs)"
                @click="onSave('1')"
                key="2"
              >
                <icon type="save" />
      ...
      >
        <a-dropdown>
          ...
          <template #overlay>
            <DataSourceList
              :show-new="showToolbarMenu(HeaderToolbarMenu.DataNew)"
              :show-online="showToolbarMenu(HeaderToolbarMenu.DataOnline)"
              :show-list="showToolbarMenu(HeaderToolbarMenu.DataList)"
            >
              <a-menu-item
                v-if="showToolbarMenu(HeaderToolbarMenu.DataRefresh)"
                @click="reloadData()"
              >
                <ReloadOutlined />
                刷新数据
      ...
      <div
        class="c-action"
        v-if="showToolbarMenu(HeaderToolbarMenu.Relationship)"
      >
      ...
      <div class="c-action" v-if="showToolbarMenu(HeaderToolbarMenu.Role)">
        <a-button type="link" @click="showDataRole = true">
          <icon type="role" />
          角色
        </a-button>
      </div>
      <div class="c-action" v-if="showToolbarMenu(HeaderToolbarMenu.Publish)">
        <a-button type="link" @click="publish()">
          <icon type="publish" />
          发布
        </a-button>
      </div>
      <div class="c-action" v-if="showToolbarMenu(HeaderToolbarMenu.Preview)">
      ...
      <div class="c-action" v-if="showToolbarMenu(HeaderToolbarMenu.Theme)">
        <a-dropdown type="link" placement="bottomCenter">
          <div
