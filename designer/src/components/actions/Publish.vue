...
export default defineComponent({
  ...
  setup(props: any, {emit}) {
    ...
    let chart = reactive({
      ...
      companyId: state.pkg.definition.companyId || 0,
    ...
    let publish = async () => {
      ...
      if (validate()) {
        try {
          ...
          state.pkg.definition.companyId = chart.companyId
          await save()

          if (!chart.overwrite) {
            // 检查重名报表
            let code = await http.request(http.option.publishCheck, {
              chart,
      ...
      try {
        await http.request(
          http.option.publish,
          {
            ...
            path: state.pkg.path,
            user: currentUser.value,
            definition: state.pkg.definition
          },
          true
        )
