...
export default defineComponent({
  ...
  setup(props: any, {emit}) {
    ...
    let sendVerCode = async () => {
      ...
      try {
        await http.request(http.option.sms, user)
    ...
    let login = async () => {
      if (validate()) {
        try {
          loading.value = true
          let result = await http.request(http.option.login, user)
          state.store = result
    ...
    let switchTo = async (user: any) => {
      let result = await http.request(
        http.option.localStorage?.setUserStore,
        user
      )
      state.store = result
    }
    ...
    let logout = async (user: any) => {
      let result = await http.request(http.option.logout, user)
      state.store = result
    }
