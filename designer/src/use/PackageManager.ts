...
export class PackageManager {
  ...
  static async loadFrom(language = 'en'): Promise<PackageManager> {
    let result = (await http.request(http.option.loadFrom, {})) as any
  ...
  static async loadFromServer(
    ...
    let result = (await http.request(http.option.loadFromServer, params)) as any
    if (result) {
      await loadPackagePlugins(result.data)
      return new PackageManager(result.data, result.path, language)
    }
  ...
  async save(showMessage = true): Promise<void> {
    ...
    await http.request(http.option.save, {
      path: this.path,
      pkg: this.definition
    })
  ...
  async saveAs(showMessage = true): Promise<void> {
    await setPackagePlugins(this.definition)
    let path = await http.request(http.option.saveAs, {
      path: this.path,
      pkg: this.definition
    })
    if (path) {
  ...
  downTable(table: ITableDefinition) {
    let index = this.definition.tables.indexOf(table)
    if (index == this.definition.tables.length - 1) {
      return
    }
    this.definition.tables.splice(index, 1)
    this.definition.tables.splice(index + 1, 0, table)
    this.addAction('down_table', table)
  }
