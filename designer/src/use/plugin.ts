...
async function openPluginFileContent() {
  let result = (await http.request(http.option.openAndReadFile, {})) as any
...
export async function addPlugin() {
  ...
  if (content) {
    ...
    if (results.plugin) {
      ...
      await http.request(http.option.localStorage.addPlugin, arg)
      let plugins = state.store[pluginType]
      if (!plugins) {
        plugins = []
      }
...
export async function removePlugin(plugin: any) {
  let state = useState()
  await http.request(http.option.localStorage.removePlugin, {
    type: pluginType,
