...
const AMapView = {
  ...
  render(amapModel, ecModel, api) {
    ...
    amap.on(
      ...
      // moveHandler
      !is2X && largeMode
        ? (moveHandler = debounce(moveHandler, 20))
        : moveHandler
    )
    ...
    if (!renderOnMoving) {
      amap.on(
        'movestart',
        (this._moveStartHandler = function (e) {
          setTimeout(function () {
...
export default isV5 ? ComponentView.extend(AMapView) : AMapView
