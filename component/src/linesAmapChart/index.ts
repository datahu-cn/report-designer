...
export class LinesAmapSeriesComponentOption {
  ...
  @ComponentControl({
    ...
    title: '是否启用大规模路径图优化',
    description:
      '开启后配合绘制优化的阈值在数据量大于指定阈值的时候对绘制进行优化, 优化生效后，动画、但路径颜色和宽度失效'
  })
  large: boolean = false
  ...
  @ComponentControl({
    ...
    show(opt: any) {
      return opt.label.show && !opt.polyline
    }
  })
  @ComponentControl({
    type: ControlType.subset,
...
export class LinesAmapChartComponentOption extends BaseComponentOption {
  ...
  dataOperation: DataOperationComponentOption = new DataOperationComponentOption(
    {_supportPartRefresh: true, _supportDrillDown: false, _supportScope: true}
  )
