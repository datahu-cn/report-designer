...
import {getContext} from './chartState'
  ...
  setup(props: any, {emit}) {
    ...
    let initChild = () => {
      ...
      for (let child of chart.children) {
        let comClass = state.components[child.type]
    ...
    let addChild = (c: any) => {
      let comClass = state.components[c.type]
      if (c.option.pos.px) {
        // 复制剪切图表时，保持原图表的像素大小
    ...
    let dropHandle = (e: MouseEvent) => {
      if (state.dragContext.target && isDragover.value) {
        let comClass = state.components[state.dragContext.target.type]
