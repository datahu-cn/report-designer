...
var LeaderLine = (function () {
  ...
  let getBodyElement = function () {
    return _parentElement || document.body
  }
  ...
  function pathDataPolyfill(window) {
    ...
    //   MIT License
    if (
      !window.SVGPathElement.prototype.getPathData ||
      !window.SVGPathElement.prototype.setPathData
    ) {
      ;(function () {
        var commandsMap = {
          Z: 'Z',
        ...
        var $cachedPathData = window.Symbol
          ? window.Symbol()
          : '__cachedPathData'
        ...
        window.SVGPathElement.prototype.removeAttribute = function (
          name,
          value
        ) {
          if (name === 'd') {
        ...
        window.SVGPathElement.prototype.getPathData = function (options) {
          ...
          if (options && options.normalize) {
            if (this[$cachedNormalizedPathData]) {
              ...
              this[$cachedNormalizedPathData] =
                clonePathData(normalizedPathData)
  ...
  function getAlpha(color) {
    ...
    if (
      (matches = /^(rgba|hsla|hwb|gray|device\-cmyk)\s*\(([\s\S]+)\)$/i.exec(
        color
      ))
    ) {
      ...
      (matches = /^\#(?:([\da-f]{6})([\da-f]{2})|([\da-f]{3})([\da-f]))$/i.exec(
        color
      ))
    ) {
      if (matches[1]) {
      ...
      over = function (event) {
        ...
        if (
          ...
          (event.relatedTarget !== this &&
            !(
              this.compareDocumentPosition(event.relatedTarget) &
  ...
  function getBBox(element, relWindow) {
    ...
    // let parent = getHtmlElement()
    // bBox.left -= parent.scrollLeft
    // bBox.right -= parent.scrollLeft
    // bBox.top -= parent.scrollTop
    // bBox.bottom -= parent.scrollTop
  ...
  function getBBoxNest(element, baseWindow) {
    ...
    if (!frames.length) {
      console.log('getBBoxNest')
      // no frame
      let bBox = getBBox(element)
      let parent = getBodyElement()
      if (parent.parentElement && parent.parentElement.scrollTop) {
        bBox.top += parent.parentElement.scrollTop
        bBox.bottom += parent.parentElement.scrollTop
      }
      if (parent.parentElement && parent.parentElement.scrollLeft) {
        bBox.left += parent.parentElement.scrollLeft
        bBox.right += parent.parentElement.scrollLeft
      }
  ...
  function getIntersection(line1P0, line1P1, line2P0, line2P1) {
    var sx1 = line1P1.x - line1P0.x,
      ...
      s =
        (-sy1 * (line1P0.x - line2P0.x) + sx1 * (line1P0.y - line2P0.y)) /
        (-sx2 * sy1 + sx1 * sy2),
      t =
        (sx2 * (line1P0.y - line2P0.y) - sy2 * (line1P0.x - line2P0.x)) /
        (-sx2 * sy1 + sx1 * sy2)

    return s >= 0 && s <= 1 && t >= 0 && t <= 1
      ? {x: line1P0.x + t * sx1, y: line1P0.y + t * sy1}
      : null
  }
  ...
  function getOffsetLine(p0, p1, offsetLen) {
    ...
    return [
      {
        x: p0.x + Math.cos(angle) * offsetLen,
        y: p0.y + Math.sin(angle) * offsetLen * -1
      },
      {
        x: p1.x + Math.cos(angle) * offsetLen,
        y: p1.y + Math.sin(angle) * offsetLen * -1
      }
    ]
  }
  ...
  function getAllPathListLen(pathList) {
    ...
    pathList.forEach(function (points) {
      var pathLen = (
        points.length === 2 ? getPointsLength : getCubicLength
      ).apply(null, points)
  ...
  function newDropShadow(document, id) {
    ...
    dropShadow.elmsAppend = [
      (dropShadow.elmFilter = filter =
        document.createElementNS(SVG_NS, 'filter'))
    ]
    ...
    filter.width.baseVal.newValueSpecifiedUnits(
      SVGLength.SVG_LENGTHTYPE_PERCENTAGE,
      100
    )
    filter.height.baseVal.newValueSpecifiedUnits(
      SVGLength.SVG_LENGTHTYPE_PERCENTAGE,
      100
    )
    filter.id = id
    ...
    if (svg2SupportedDropShadow) {
      ...
      dropShadow.elmBlur = filter.appendChild(
        document.createElementNS(SVG_NS, 'feGaussianBlur')
      )
      dropShadow.elmOffset = element = filter.appendChild(
        document.createElementNS(SVG_NS, 'feOffset')
      )
      element.result.baseVal = 'offsetblur'
      element = filter.appendChild(document.createElementNS(SVG_NS, 'feFlood'))
      dropShadow.styleFlood = element.style
      element = filter.appendChild(
        document.createElementNS(SVG_NS, 'feComposite')
      )
      element.in2.baseVal = 'offsetblur'
      element.operator.baseVal =
      ...
      element.appendChild(
        document.createElementNS(SVG_NS, 'feMergeNode')
  ...
  function getBodyOffset(window) {
    ...
    var stylesHtml = window.getComputedStyle(getHtmlElement(), ''),
      stylesBody = window.getComputedStyle(getBodyElement(), ''),
      bodyOffset = {x: 0, y: 0}
    ...
    if (stylesBody.position !== 'static') {
      ...
      bodyOffset.y -= [stylesHtml.marginTop, stylesHtml.borderTopWidth].reduce(
        sumProps,
        0
      )
    }
    let rect = getBodyElement().getBoundingClientRect()
    bodyOffset.x = bodyOffset.x - rect.x - getHtmlElement().scrollLeft
    bodyOffset.y = bodyOffset.y - rect.y - getHtmlElement().scrollTop
    return bodyOffset
  }
  ...
  function setupWindow(window) {
    ...
    if (!baseDocument.getElementById(DEFS_ID)) {
      // Add svg defs
      defsSvg = new window.DOMParser().parseFromString(
        DEFS_HTML,
        'image/svg+xml'
      )
      getBodyElement().appendChild(defsSvg.documentElement)
  ...
  function bindWindow(props, newWindow) {
    ...
    function setupMask(id) {
      var element = defs.appendChild(
        baseDocument.createElementNS(SVG_NS, 'mask')
      )
      element.id = id
      element.maskUnits.baseVal = SVGUnitTypes.SVG_UNIT_TYPE_USERSPACEONUSE
      ;[element.x, element.y, element.width, element.height].forEach(function (
        len
      ) {
    ...
    function setupMarker(id) {
      var element = defs.appendChild(
        baseDocument.createElementNS(SVG_NS, 'marker')
      )
      element.id = id
    ...
    function setWH100(element) {
      ;[element.width, element.height].forEach(function (len) {
        len.baseVal.newValueSpecifiedUnits(
          SVGLength.SVG_LENGTHTYPE_PERCENTAGE,
          100
        )
      })
    ...
    if (props.baseWindow && props.svg) {
      getBodyElement().removeChild(props.svg)
    }
    ...
    props.defs = defs = svg.appendChild(
      baseDocument.createElementNS(SVG_NS, 'defs')
    )

    props.linePath = element = defs.appendChild(
      baseDocument.createElementNS(SVG_NS, 'path')
    )
    ...
    props.lineShape = element = defs.appendChild(
    ...
    props.capsMaskAnchorSE = [0, 1].map(function () {
      var element = maskCaps.appendChild(
        baseDocument.createElementNS(SVG_NS, 'path')
      )
    ...
    props.lineMaskMarkerIdSE = [
      prefix + '-caps-mask-marker-0',
      prefix + '-caps-mask-marker-1'
    ]
    ...
    props.capsMaskMarkerShapeSE = [0, 1].map(function (i) {
      var element = props.capsMaskMarkerSE[i].appendChild(
        baseDocument.createElementNS(SVG_NS, 'use')
      )
    ...
    props.capsMaskLine = element = maskCaps.appendChild(
      baseDocument.createElementNS(SVG_NS, 'use')
    )
    element.className.baseVal = APP_ID + '-caps-mask-line'
    element.href.baseVal = '#' + lineShapeId

    props.maskBGRect = element = setWH100(
      defs.appendChild(baseDocument.createElementNS(SVG_NS, 'rect'))
    )
    ...
    // lineMask
    props.lineMask = setWH100(
      setupMask((props.lineMaskId = prefix + '-line-mask'))
    )
    props.lineMaskBG = element = props.lineMask.appendChild(
      baseDocument.createElementNS(SVG_NS, 'use')
    )
    element.href.baseVal = '#' + maskBGRectId
    props.lineMaskShape = element = props.lineMask.appendChild(
      baseDocument.createElementNS(SVG_NS, 'use')
    )
    element.className.baseVal = APP_ID + '-line-mask-shape'
    element.href.baseVal = '#' + linePathId
    element.style.display = 'none'
    props.lineMaskCaps = element = props.lineMask.appendChild(
      baseDocument.createElementNS(SVG_NS, 'use')
    )
    element.href.baseVal = '#' + capsId

    // lineOutlineMask
    props.lineOutlineMask = setWH100(
      setupMask((lineOutlineMaskId = prefix + '-line-outline-mask'))
    )
    element = props.lineOutlineMask.appendChild(
      baseDocument.createElementNS(SVG_NS, 'use')
    )
    element.href.baseVal = '#' + maskBGRectId
    ...
    */
    ...
    props.lineFace = element = props.face.appendChild(
      baseDocument.createElementNS(SVG_NS, 'use')
    )
    element.href.baseVal = '#' + lineShapeId

    props.lineOutlineFace = element = props.face.appendChild(
      baseDocument.createElementNS(SVG_NS, 'use')
    )
    element.href.baseVal = '#' + lineShapeId
    ...
    props.plugMaskShapeSE = [0, 1].map(function (i) {
      var element = props.plugMaskSE[i].appendChild(
        baseDocument.createElementNS(SVG_NS, 'use')
      )
    ...
    props.plugOutlineMaskSE = [0, 1].map(function (i) {
      return setupMask(
        (plugOutlineMaskIdSE[i] = prefix + '-plug-outline-mask-' + i)
      )
    })
    props.plugOutlineMaskShapeSE = [0, 1].map(function (i) {
      var element = props.plugOutlineMaskSE[i].appendChild(
        baseDocument.createElementNS(SVG_NS, 'use')
      )
    ...
    props.plugMarkerIdSE = [
      prefix + '-plug-marker-0',
      prefix + '-plug-marker-1'
    ]
    props.plugMarkerSE = [0, 1].map(function (i) {
      ...
      if (IS_WEBKIT) {
        ...
        element.markerUnits.baseVal =
          SVGMarkerElement.SVG_MARKERUNITS_USERSPACEONUSE
      }
      return element
    })
    props.plugMarkerShapeSE = [0, 1].map(function (i) {
      return props.plugMarkerSE[i].appendChild(
        baseDocument.createElementNS(SVG_NS, 'g')
      )
    })

    props.plugFaceSE = [0, 1].map(function (i) {
      return props.plugMarkerShapeSE[i].appendChild(
        baseDocument.createElementNS(SVG_NS, 'use')
      )
    })
    props.plugOutlineFaceSE = [0, 1].map(function (i) {
      var element = props.plugMarkerShapeSE[i].appendChild(
        baseDocument.createElementNS(SVG_NS, 'use')
      )
    ...
    props.plugsFace = element = props.face.appendChild(
      baseDocument.createElementNS(SVG_NS, 'use')
    )
    ...
    getBodyElement().appendChild(svg)
  ...
  function updateLine(props) {
    ...
    updated =
      setStat(
        ...
        curStats,
        'line_colorTra',
        getAlpha(curStats.line_color)[0] < 1
      ) || updated
    updated =
      setStat(
        props,
  ...
  function updatePlug(props) {
    ...
    ;[0, 1].forEach(function (i) {
      ...
      curStats.plugOutline_plugSE[i] = curStats.capsMaskMarker_plugSE[i] =
        plugId
      if (curStats.plug_enabledSE[i]) {
        value =
          (curStats.line_strokeWidth / DEFAULT_OPTIONS.lineSize) *
          options.plugSizeSE[i]
  ...
  function updateLineOutline(props) {
    ...
    updated =
      setStat(props, curStats, 'lineOutline_color', options.lineOutlineColor) ||
      updated
    updated =
      setStat(
        props,
  ...
  function updatePlugOutline(props) {
    ...
    ;[0, 1].forEach(function (i) {
      var plugId = curStats.plugOutline_plugSE[i],
        symbolConf =
          plugId !== PLUG_BEHIND ? SYMBOLS[PLUG_2_SYMBOL[plugId]] : null,
        value
    ...
    if (curStats.plug_enabled) {
      ;[0, 1].forEach(function (i) {
        ...
        if (
          ...
          props.plugsFace.style[marker.prop] = value
            ? 'url(#' + props.plugMarkerIdSE[i] + ')'
            : 'none'
          updated = true
        }
        ...
        if (curStats.plug_enabledSE[i]) {
          ...
          if (
            ...
            updated = true
            if (
              (IS_BLINK || IS_WEBKIT || IS_TRIDENT) &&
              !curStats.line_colorTra
            ) {
              // [BLINK], [WEBKIT], [TRIDENT] capsMaskMarkerShapeSE is not updated when line has no alpha
              forceReflowAdd(
                props,
                IS_TRIDENT ? props.lineMaskCaps : props.capsMaskLine
              )
            }
          }
          ...
          if (
            ...
            if (value) {
              props.plugFaceSE[i].style.mask =
                'url(#' + props.plugMaskIdSE[i] + ')'
          ...
          if (curStats.plugOutline_enabledSE[i]) {
            if (
              ...
              props.plugOutlineFaceSE[i].href.baseVal =
                ...
                props.plugOutlineMaskShapeSE[i].href.baseVal =
                  '#' + symbolConf.elmId
              ;[props.plugMaskSE[i], props.plugOutlineMaskSE[i]].forEach(
                function (mask) {
            ...
            if (
              setStat(
                ...
                /* [DEBUG] */ 'plugOutline_strokeWidthSE[' +
                  i +
                  ']%_' /* [/DEBUG] */
              )
            ) {
            ...
            if (
              setStat(
                ...
                /* [DEBUG] */ 'plugOutline_inStrokeWidthSE[' +
                  i +
                  ']%_' /* [/DEBUG] */
              )
            ) {
  ...
  function updatePosition(props) {
    ...
    curStats.position_socketGravitySE = curSocketGravitySE = copyTree(
    ...
    ;(function () {
      ...
      if (options.socketSE[0] && options.socketSE[1]) {
        ...
        if (!options.socketSE[0] && !options.socketSE[1]) {
          ...
          curSocketXYSE[iFix] = getSocketXY(
            anchorBBoxSE[iFix],
            options.socketSE[iFix]
          )
          socketXYsWk = SOCKET_IDS.map(function (socketId) {
        ...
        ;[0, 1].forEach(function (i) {
          ...
          if (!options.socketSE[i]) {
            if (!anchorBBoxSE[i].width && !anchorBBoxSE[i].height) {
              ...
              (curSocketXYSE[i].socketId === SOCKET_LEFT ||
                curSocketXYSE[i].socketId === SOCKET_RIGHT)
            ) {
              curSocketXYSE[i].socketId =
                curSocketXYSE[i ? 0 : 1].x - anchorBBoxSE[i].left >= 0
                  ? SOCKET_RIGHT
                  : SOCKET_LEFT
            } else if (
              !anchorBBoxSE[i].height &&
              (curSocketXYSE[i].socketId === SOCKET_TOP ||
                curSocketXYSE[i].socketId === SOCKET_BOTTOM)
            ) {
              curSocketXYSE[i].socketId =
                curSocketXYSE[i ? 0 : 1].y - anchorBBoxSE[i].top >= 0
                  ? SOCKET_BOTTOM
                  : SOCKET_TOP
            }
          }
    ...
    ;[0, 1].forEach(function (i) {
      if (
        curStats.position_plugOverheadSE[i] !==
        aplStats.position_plugOverheadSE[i]
      ) {
      ...
      if (
        socketGravityHasChanged(
          curSocketGravitySE[i],
          aplStats.position_socketGravitySE[i]
        )
      ) {
    ...
    if (
      ...
      [0, 1].some(function (i) {
        return (
          ...
          socketXYHasChanged(
            curSocketXYSE[i],
            aplStats.position_socketXYSE[i]
          ) ||
          socketGravityHasChanged(
            curSocketGravitySE[i],
            aplStats.position_socketGravitySE[i]
      ...
      switch (curStats.position_path) {
        ...
        case PATH_ARC:
          ;(function () {
            var downward =
                (typeof curSocketGravitySE[0] === 'number' &&
                  curSocketGravitySE[0] > 0) ||
                (typeof curSocketGravitySE[1] === 'number' &&
                  curSocketGravitySE[1] > 0),
              circle8rad = CIRCLE_8_RAD * (downward ? -1 : 1),
              angle = Math.atan2(
                curSocketXYSE[1].y - curSocketXYSE[0].y,
                curSocketXYSE[1].x - curSocketXYSE[0].x
              ),
              cp1Angle = -angle + circle8rad,
              cp2Angle = Math.PI - angle - circle8rad,
              crLen =
                (getPointsLength(curSocketXYSE[0], curSocketXYSE[1]) /
                  Math.sqrt(2)) *
                CIRCLE_CP,
              cp1 = {
        ...
        case PATH_FLUID:
        case PATH_MAGNET:
          /* @EXPORT[file:../test/spec/func/PATH_FLUID]@ */ ;(function (
            socketGravitySE
          ) {
            var cx = [],
              cy = []
            curSocketXYSE.forEach(function (socketXY, i) {
              ...
              if (Array.isArray(gravity)) {
                ...
                minGravity =
                  overhead > 0
                    ? MIN_OH_GRAVITY +
                      (overhead > MIN_OH_GRAVITY_OH
                        ? (overhead - MIN_OH_GRAVITY_OH) * MIN_OH_GRAVITY_R
                        : 0)
                    : MIN_GRAVITY +
                      (curStats.position_lineStrokeWidth > MIN_GRAVITY_SIZE
                        ? (curStats.position_lineStrokeWidth -
                            MIN_GRAVITY_SIZE) *
                          MIN_GRAVITY_R
                        : 0)
          ...
          /* @EXPORT[file:../test/spec/func/PATH_GRID]@ */ ;(function () {
            ...
            function joinPoints() {
              var scopeContains = [
                ...
                axis = [
                  getAxis(curDirPoint[0].dirId),
                  getAxis(curDirPoint[1].dirId)
                ],
                center,
                axisScope,
                distance,
                ...
                if (scopeContains[0] && scopeContains[1]) {
                  if (!onAxisLine(curDirPoint[1], curDirPoint[0])) {
                    if (curDirPoint[0][axis[0]] === curDirPoint[1][axis[1]]) {
                      ...
                      center =
                        curDirPoint[0][axis[0]] +
                        (curDirPoint[1][axis[1]] - curDirPoint[0][axis[0]]) / 2
                      dpList[0].push(
                        getNextDirPoint(
                          curDirPoint[0],
                          Math.abs(center - curDirPoint[0][axis[0]])
                        )
                      )
                      dpList[1].push(
                        getNextDirPoint(
                          curDirPoint[1],
                          Math.abs(center - curDirPoint[1][axis[1]])
                        )
                      )
                    }
                  }
                  ...
                  dpList[axisScope.notContain].push(
                    curDirPoint[axisScope.notContain]
                  )
                  curDirPoint[axisScope.notContain] = getNextDirPoint(
                    curDirPoint[axisScope.notContain],
        ...
        curStats.position_plugOverheadSE.forEach(function (plugOverhead, i) {
          ...
          if (plugOverhead > 0) {
            ...
            if (pathPoints.length === 2) {
              // Straight line
              pathSegsLen[iSeg] =
                pathSegsLen[iSeg] || getPointsLength.apply(null, pathPoints)
              ...
              // Cubic bezier
              pathSegsLen[iSeg] =
                pathSegsLen[iSeg] || getCubicLength.apply(null, pathPoints)
              if (pathSegsLen[iSeg] > MIN_ADJUST_LEN) {
                if (pathSegsLen[iSeg] - plugOverhead < MIN_ADJUST_LEN) {
                  plugOverhead = pathSegsLen[iSeg] - MIN_ADJUST_LEN
                }
            ...
            axis =
              socketId === SOCKET_LEFT || socketId === SOCKET_RIGHT ? 'x' : 'y'
            minAdjustOffset =
              -anchorBBoxSE[i][axis === 'x' ? 'width' : 'height']
            if (plugOverhead < minAdjustOffset) {
              plugOverhead = minAdjustOffset
            }
            dir =
              plugOverhead *
              (socketId === SOCKET_LEFT || socketId === SOCKET_TOP ? -1 : 1)
            if (pathPoints.length === 2) {
              // Straight line
              ...
              ;(start
                ? [0, 1]
                : [pathPoints.length - 2, pathPoints.length - 1]
              ).forEach(function (i) {
      ...
      aplStats.position_plugOverheadSE = copyTree(
        curStats.position_plugOverheadSE
      )
  ...
  function updatePath(props) {
    ...
    if (pathList) {
      ...
      curStats.path_pathData = curPathData = pathList2PathData(
        pathList,
        function (point) {
  ...
  function updateViewBox(props) {
    ...
    var curStats = props.curStats,
      ...
      viewBox = props.svg.viewBox.baseVal,
    ...
    curBBox.x =
      curStats.lineMask_x =
      curStats.lineOutlineMask_x =
      curStats.maskBGRect_x =
        edge.x1
    curBBox.y =
      curStats.lineMask_y =
      curStats.lineOutlineMask_y =
      curStats.maskBGRect_y =
        edge.y1
    ...
    ;['x', 'y', 'width', 'height'].forEach(function (boxKey) {
      ...
      if ((value = curBBox[boxKey]) !== aplBBox[boxKey]) {
        ...
        styles[BBOX_PROP[boxKey]] =
          value +
          (boxKey === 'x' || boxKey === 'y' ? props.bodyOffset[boxKey] : 0) +
          'px'
        updated = true
      }
    })
  ...
  function updateMask(props) {
    ...
    if (curStats.plug_enabled) {
      ;[0, 1].forEach(function (i) {
        curStats.capsMaskMarker_enabledSE[i] =
          ...
          (curStats.plugOutline_enabledSE[i] &&
            curStats.plugOutline_colorTraSE[i])
      })
    } else {
      curStats.capsMaskMarker_enabledSE[0] =
        curStats.capsMaskMarker_enabledSE[1] = false
    }
    curStats.capsMaskMarker_enabled =
      curStats.capsMaskMarker_enabledSE[0] ||
      curStats.capsMaskMarker_enabledSE[1]
    ...
    curStats.caps_enabled =
      curStats.capsMaskMarker_enabled ||
      curStats.capsMaskAnchor_enabledSE[0] ||
      curStats.capsMaskAnchor_enabledSE[1]
    curStats.lineMask_enabled =
      curStats.caps_enabled || curStats.lineMask_outlineMode
    lineMaskBGEnabled =
      curStats.lineMask_enabled && !curStats.lineMask_outlineMode
    ...
    if (
      ...
      props.lineFace.style.mask = value
        ? 'url(#' + props.lineMaskId + ')'
        : 'none'
      updated = true
      if (IS_WEBKIT) {
    ...
    if (curStats.lineMask_enabled) {
      ...
      if (
        ...
        props.lineMaskCaps.style.display =
          props.lineOutlineMaskCaps.style.display = value ? 'inline' : 'none'
        updated = true
        if (IS_WEBKIT) {
        ...
        if (curStats.capsMaskMarker_enabled) {
          ...
          ;[0, 1].forEach(function (i) {
            ...
            if (
              ...
              props.capsMaskLine.style[marker.prop] = value
                ? 'url(#' + props.lineMaskMarkerIdSE[i] + ')'
                : 'none'
              updated = true
            }
            ...
            if (curStats.capsMaskMarker_enabledSE[i]) {
              ...
              if (
                ...
                props.capsMaskMarkerShapeSE[i].href.baseVal =
                  '#' + symbolConf.elmId
                setMarkerOrient(
                  props,
  ...
  function setEffect(props) {
    ...
    Object.keys(EFFECTS).forEach(function (effectName) {
      ...
      if (
        setStat(props, aplStats, keyEnabled, (enabled = curStats[keyEnabled]))
      ) {
        // ON/OFF
        if (enabled) {
  ...
  function getValidAnimOptions(animOptions, defaultAnimOptions) {
    return {
      duration:
        isFinite(animOptions.duration) && animOptions.duration > 0
          ? animOptions.duration
          : defaultAnimOptions.duration,
      timing: anim.validTiming(animOptions.timing)
        ? animOptions.timing
        : copyTree(defaultAnimOptions.timing)
    }
  }
  ...
  function show(props, on, showEffectName, animOptions) {
    ...
    update.show_animOptions = hasChanged(
      curStats.show_animOptions,
      aplStats.show_animOptions
    )
  ...
  function setOptions(props, newOptions) {
    ...
    */
    ...
    function setValidId(
      ...
      if (
        curOption.container[curOption.key] == null &&
        !curOption.acceptsAuto
      ) {
    ...
    function setValidType(
      ...
      if (
        newOptions[propName] != null &&
        ((curOption.acceptsAuto &&
          (newOptions[propName] + '').toLowerCase() === KEYWORD_AUTO) ||
          (isValidType((value = newOptions[propName]), type) &&
            ((value =
              trim && type === 'string' && value ? value.trim() : value) ||
              true) &&
            (!check || check(value)))) &&
    ...
    ;['start', 'end'].forEach(function (optionName, i) {
      ...
      if (
        ...
        if (
          newIsAttachment &&
          !bindAttachment(props, insAttachProps[newOption._id], optionName)
        ) {
          // Bind new
    ...
    if (
      !options.anchorSE[0] ||
      !options.anchorSE[1] ||
      options.anchorSE[0] === options.anchorSE[1]
    ) {
    ...
    // socketGravitySE
    ;[newOptions.startSocketGravity, newOptions.endSocketGravity].forEach(
        ...
        if (newOption != null) {
          if (Array.isArray(newOption)) {
            if (isFinite(newOption[0]) && isFinite(newOption[1])) {
              ...
              if (
                Array.isArray(options.socketGravitySE[i]) &&
    ...
    ;['startLabel', 'endLabel', 'middleLabel'].forEach(function (
      ...
      if (
        (plain ||
          (newOption &&
            (newIsAttachment = isAttachment(newOption, 'label')))) &&
        newOption !== oldOption
      ) {
        ...
        if (newOption) {
          if (newIsAttachment) {
            ...
            label = new LeaderLineAttachment(ATTACHMENTS.captionLabel, [
              newOption
            ])
          }
    ...
    Object.keys(EFFECTS).forEach(function (effectName) {
      ...
      function parseAnimOptions(newEffectOptions) {
        ...
        if (!newEffectOptions.hasOwnProperty('animation')) {
          ...
          props.curStats[keyAnimOptions] = optionValue
            ? getValidAnimOptions({}, effectConf.defaultAnimOptions)
            : null
  ...
  EFFECTS = {
    dash: {
      ...
      update: function (props) {
        ...
        update =
          setStat(props, aplStats, 'dash_len', curStats.dash_len) || update
        update =
          setStat(props, aplStats, 'dash_gap', curStats.dash_gap) || update
        if (update) {
          props.lineFace.style.strokeDasharray =
            aplStats.dash_len + ',' + aplStats.dash_gap
        }
        ...
        if (curStats.dash_animOptions) {
          ...
          if (
            ...
            (update ||
              hasChanged(curStats.dash_animOptions, aplStats.dash_animOptions))
          ) {
    ...
    gradient: {
      stats: {
        gradient_colorSE: {hasSE: true},
        gradient_pointSE: {hasSE: true, hasProps: true}
      },
      ...
      init: function (props) {
        ...
        element.id = id
        element.gradientUnits.baseVal =
          SVGUnitTypes.SVG_UNIT_TYPE_USERSPACEONUSE
        ;[element.x1, element.y1, element.x2, element.y2].forEach(function (
          len
        ) {
          len.baseVal.newValueSpecifiedUnits(SVGLength.SVG_LENGTHTYPE_PX, 0)
        })
        props.efc_gradient_stopSE = [0, 1].map(function (i) {
          var element = props.efc_gradient_gradient.appendChild(
            baseDocument.createElementNS(SVG_NS, 'stop')
          )
          try {
      ...
      update: function (props) {
        ...
        var curStats = props.curStats,
          ...
          point
        ;[0, 1].forEach(function (i) {
          curStats.gradient_colorSE[i] =
            effectOptions.colorSE[i] || curStats.plug_colorSE[i]
        })
        ...
        ;[0, 1].forEach(function (i) {
          ...
          ;['x', 'y'].forEach(function (pointKey) {
            if (
              (value = curStats.gradient_pointSE[i][pointKey]) !==
              aplStats.gradient_pointSE[i][pointKey]
            ) {
  ...
  SHOW_EFFECTS = {
    ...
    fade: {
      ...
      stop: function (props, finish, on) {
        ...
        on = on != null ? on : props.aplStats.show_on
        traceLog.add(dbgLog + '=' + on) // [DEBUG/]
        timeRatio = curStats.show_inAnim
          ? anim.stop(curStats.show_animId)
          : on
          ? 1
          : 0
        curStats.show_inAnim = false
  ...
  function LeaderLine(start, end, options) {
    ...
    props.curStats.show_animOptions = copyTree(
      SHOW_EFFECTS[DEFAULT_SHOW_EFFECT].defaultAnimOptions
    )
  ...
  ;(function () {
    ...
    Object.keys(EFFECTS).forEach(function (effectName) {
      ...
      function getOptions(optionValue) {
        var effectOptions = effectConf.optionsConf.reduce(function (
          effectOptions,
          optionConf
        ) {
          var optionClass = optionConf[0],
  ...
  ATTACHMENTS = {
    pointAnchor: {
      ...
      init: function (attachProps, attachOptions) {
        ...
        attachProps.element = ATTACHMENTS.pointAnchor.checkElement(
          attachOptions.element
        )
        attachProps.x = ATTACHMENTS.pointAnchor.parsePercent(
          attachOptions.x,
          true
        ) || [0.5, true]
        attachProps.y = ATTACHMENTS.pointAnchor.parsePercent(
          attachOptions.y,
          true
        ) || [0.5, true]
        traceLog.add('</ATTACHMENTS.pointAnchor.init>') // [DEBUG/]
      ...
      removeOption: function (attachProps, boundTarget) {
        ...
        var props = boundTarget.props,
          ...
          another =
            props.options.anchorSE[boundTarget.optionName === 'start' ? 1 : 0]
        if (element === another) {
          // must be not another
          element =
            another === getBodyElement()
              ? new LeaderLineAttachment(ATTACHMENTS.pointAnchor, [element])
              : getBodyElement()
        }
      ...
      getBBoxNest: function (attachProps, props) {
        ...
        bBox.left = bBox.right =
          bBox.left + attachProps.x[0] * (attachProps.x[1] ? width : 1)
        bBox.top = bBox.bottom =
          bBox.top + attachProps.y[0] * (attachProps.y[1] ? height : 1)
        return bBox
      },
    ...
    areaAnchor: {
      ...
      init: function (attachProps, attachOptions) {
        ...
        if (attachOptions.shape === 'circle') {
          ...
          attachOptions.points.every(function (point) {
            var validPoint = {}
            if (
              (validPoint.x = ATTACHMENTS.pointAnchor.parsePercent(
                point[0],
                true
              )) &&
              (validPoint.y = ATTACHMENTS.pointAnchor.parsePercent(
                point[1],
                true
              ))
            ) {
          ...
          attachProps.radius =
            isFinite(attachOptions.radius) && attachOptions.radius >= 0
              ? attachOptions.radius
              : 0
        }
        ...
        if (attachProps.shape === 'rect' || attachProps.shape === 'circle') {
          ...
          attachProps.width = ATTACHMENTS.pointAnchor.parsePercent(
            attachOptions.width
          ) || [1.1, true]
          attachProps.height = ATTACHMENTS.pointAnchor.parsePercent(
            attachOptions.height
          ) || [1.1, true]
          if (
            attachProps.x[1] ||
            attachProps.y[1] ||
            attachProps.width[1] ||
            attachProps.height[1]
          ) {
            attachProps.hasRatio = true
          }
        }
        ...
        attachProps.path = svg.appendChild(
          baseDocument.createElementNS(SVG_NS, 'path')
        )
        ...
        attachProps.updateColor = function () {
          ...
          var curStats = attachProps.curStats,
            ...
            llStats = attachProps.boundTargets.length
              ? attachProps.boundTargets[0].props.curStats
              : null,
            value

          curStats.color = value =
            attachProps.color ||
            (llStats ? llStats.line_color : DEFAULT_OPTIONS.lineColor)
      ...
      getStrokeWidth: function (attachProps, props) {
        ...
        if (
          ...
          attachProps.boundTargets.length > 1
        ) {
          traceLog.add('update-boundTargets') // [DEBUG/]
          addDelayedProc(function () {
            attachProps.boundTargets.forEach(function (boundTarget) {
      ...
      getPathData: function (attachProps, props) {
        var bBox = getBBoxNest(attachProps.element, props.baseWindow)
        return pathList2PathData(
          attachProps.curStats.pathListRel,
          function (point) {
            point.x += bBox.left
            point.y += bBox.top
          }
      ...
      update: function (attachProps) {
        ...
        updated.strokeWidth = setStat(
          ...
          curStats,
          'strokeWidth',
          attachProps.size != null
            ? attachProps.size
            : llStats
            ? llStats.line_strokeWidth
            : DEFAULT_OPTIONS.lineSize
        )

        elementBBox = getBBox(attachProps.element)
        updated.elementWidth = setStat(
          attachProps,
          curStats,
          'elementWidth',
          elementBBox.width
        )
        updated.elementHeight = setStat(
          attachProps,
          curStats,
          'elementHeight',
          elementBBox.height
        )
        updated.elementLeft = setStat(
          attachProps,
          curStats,
          'elementLeft',
          elementBBox.left
        )
        updated.elementTop = setStat(
          attachProps,
          curStats,
          'elementTop',
          elementBBox.top
        ...
        if (
          updated.strokeWidth ||
          (attachProps.hasRatio &&
            (updated.elementWidth || updated.elementHeight))
        ) {
          // generate path
            ...
            case 'rect':
              ;(function () {
                ...
                areaBBox = {
                  left:
                    attachProps.x[0] *
                    (attachProps.x[1] ? elementBBox.width : 1),
                  top:
                    attachProps.y[0] *
                    (attachProps.y[1] ? elementBBox.height : 1),
                  width:
                    attachProps.width[0] *
                    (attachProps.width[1] ? elementBBox.width : 1),
                  height:
                    attachProps.height[0] *
                    (attachProps.height[1] ? elementBBox.height : 1)
                }
                ...
                maxRadius = (side = Math.min(areaBBox.width, areaBBox.height))
                  ...
                  : 0
                radius = !attachProps.radius
                  ? 0
                  : attachProps.radius <= maxRadius
                  ? attachProps.radius
                  : maxRadius
                if (radius) {
                  ...
                  points = [
                    ...
                    {
                      x: areaBBox.right + padding,
                      y: areaBBox.bottom - offsetC
                    }, // 4 right-bottom-start
                    {
                      x: areaBBox.right - offsetC,
                      y: areaBBox.bottom + padding
                    }, // 5 right-bottom-end
                    {
                      x: areaBBox.left + offsetC,
                      y: areaBBox.bottom + padding
                    }, // 6 left-bottom-start
                    {
                      x: areaBBox.left - padding,
                      y: areaBBox.bottom - offsetC
                    } // 7 left-bottom-end
                  ]
            ...
            case 'circle':
              ;(function () {
                ...
                points = [
                  ...
                  {
                    x: areaBBox.right + paddingX,
                    y: areaBBox.bottom + paddingY
                  }
                ] // right-bottom
                curStats.bBoxRel = {
                  left: points[0].x,
        ...
        if (updated.pathListRel || updated.elementLeft || updated.elementTop) {
          curStats.pathData = pathList2PathData(
            curStats.pathListRel,
            function (point) {
        ...
        // Apply `pathData`
        if (
          pathDataHasChanged((value = curStats.pathData), aplStats.pathData)
        ) {
        ...
        if (attachProps.dash) {
          if (
            updated.pathData ||
            (updated.strokeWidth &&
              (!attachProps.dashLen || !attachProps.dashGap))
          ) {
          ...
          updated.dash =
            setStat(attachProps, aplStats, 'dashLen', curStats.dashLen) ||
            updated.dash
          updated.dash =
            setStat(attachProps, aplStats, 'dashGap', curStats.dashGap) ||
            updated.dash
          if (updated.dash) {
            attachProps.path.style.strokeDasharray =
              aplStats.dashLen + ',' + aplStats.dashGap
          }
        }
        ...
        ;(function () {
          var curVBBBox = curStats.viewBoxBBox,
            ...
            viewBox = attachProps.svg.viewBox.baseVal,
    ...
    mouseHoverAnchor: {
      ...
      padding: {
        top: 1,
        right: 15 /* >(backgroundSize.width + backgroundPosition.right) */,
        bottom: 1,
        left: 2
      },
      minHeight: 15,
      ...
      init: function (attachProps, attachOptions) {
        ...
        ;['style', 'hoverStyle'].forEach(function (key) {
          ...
          attachProps[key] = Object.keys(defaultStyle).reduce(function (
            copyObj,
            propName
          ) {
        ...
        curStyle =
          attachProps.element.ownerDocument.defaultView.getComputedStyle(
            attachProps.element,
            ''
          )
        // display
        ...
        if (bBox.height < conf.minHeight) {
          if (IS_TRIDENT) {
            ...
            attachProps.style.height =
              parseFloat(curStyle.height) +
              (conf.minHeight - bBox.height) +
              'px'
          }
        }
        ...
        ;['style', 'hoverStyle'].forEach(function (key) {
          ...
          if (isObject(optionStyle)) {
            Object.keys(optionStyle).forEach(function (propName) {
              if (
                typeof optionStyle[propName] === 'string' ||
                isFinite(optionStyle[propName])
              ) {
        ...
        if (
          attachOptions.showEffectName &&
          SHOW_EFFECTS[attachOptions.showEffectName]
        ) {
          attachProps.showEffectName = showEffectName =
            attachOptions.showEffectName
        }
        ...
        attachProps.mouseenter = function (event) {
          ...
          attachProps.hoverStyleSave = conf.getStyles(
            elmStyle,
            Object.keys(attachProps.hoverStyle)
          )
      ...
      bind: function (attachProps, bindTarget) {
        ...
        if (bindTarget.props.svg) {
          ATTACHMENTS.mouseHoverAnchor.llShow(
            bindTarget.props,
            false,
            attachProps.showEffectName
          )
        } else {
        ...
        if (!attachProps.enabled) {
          ...
          ATTACHMENTS.mouseHoverAnchor.setStyles(
            attachProps.elmStyle,
            attachProps.style
          )
      ...
      llShow: function (props, on, showEffectName) {
        SHOW_EFFECTS[showEffectName || props.curStats.show_effect].stop(
          props,
          true,
          on
        )
    ...
    captionLabel: {
      ...
      init: function (attachProps, attachOptions) {
        ...
        attachProps.outlineColor =
          typeof attachOptions.outlineColor === 'string'
            ? attachOptions.outlineColor.trim()
            : '#fff' // default
        if (
          ...
          attachProps.offset = {
            x: attachOptions.offset[0],
            y: attachOptions.offset[1]
          }
        }
        ...
        attachProps.updateSocketXY = function (props) {
          ...
          if (attachProps.offset) {
            ...
            anotherSocketXY =
              llStats.position_socketXYSE[attachProps.socketIndex ? 0 : 1]
            if (
              socketXY.socketId === SOCKET_LEFT ||
              socketXY.socketId === SOCKET_RIGHT
            ) {
              curStats.x =
                socketXY.socketId === SOCKET_LEFT
                  ? socketXY.x - margin - attachProps.width
                  : socketXY.x + margin
              curStats.y =
              ...
              curStats.y =
                socketXY.socketId === SOCKET_TOP
                  ? socketXY.y - margin - attachProps.height
                  : socketXY.y + margin
            }
          }
          ...
          if (
            ...
            attachProps.elmPosition.y.baseVal.getItem(0).value =
              value + attachProps.height
          }
          traceLog.add('</ATTACHMENTS.captionLabel.updateSocketXY>') // [DEBUG/]
        }
        ...
        attachProps.updatePath = function (props) {
          ...
          point = ATTACHMENTS.captionLabel.getMidPoint(
            pathList,
            attachProps.lineOffset
          )
            ...
            if (curStats.x != null) {
              ATTACHMENTS.captionLabel.adjustEdge(
                edge,
                {
                  x: curStats.x,
                  y: curStats.y,
                  width: attachProps.width,
                  height: attachProps.height
                },
      ...
      initSvg: function (attachProps, props) {
        ...
        if (attachProps.outlineColor) {
          ...
          strokeWidth =
            strokeWidth > 10 ? 10 : strokeWidth < 2 ? 2 : strokeWidth
      ...
      bind: function (attachProps, bindTarget) {
        ...
        if (
          (attachProps.refSocketXY =
            bindTarget.optionName === 'startLabel' ||
            bindTarget.optionName === 'endLabel')
        ) {
          attachProps.socketIndex =
            bindTarget.optionName === 'startLabel' ? 0 : 1
    ...
    pathLabel: {
      ...
      init: function (attachProps, attachOptions) {
        ...
        attachProps.updateStartOffset = function (props) {
          ...
          pathLenAll = getAllPathDataLen(curStats.pathData)
          startOffset =
            attachProps.semIndex === 0
              ? 0
              : attachProps.semIndex === 1
              ? pathLenAll
              : pathLenAll / 2
          if (attachProps.semIndex !== 2) {
            plugBackLen =
              ...
              attachProps.height / 4
            startOffset +=
              attachProps.semIndex === 0 ? plugBackLen : -plugBackLen
            startOffset =
              startOffset < 0
                ? 0
                : startOffset > pathLenAll
                ? pathLenAll
                : startOffset
          }
            ...
            if (lastLineSeg) {
              ...
              if (angle >= 0 && angle <= Math.PI) {
                ...
                if (
                  ...
                  lineSeg = {
                    type: 'line',
                    points: [intPoint, offsetPoints[1]]
                  }
                } else {
                  lastPoints[1] = nearPoints(exPoint, exPointLast)
                    ? exPoint
                    : exPointLast
                  lineSeg = {
                    type: 'line',
                    points: [exPoint, offsetPoints[1]]
                  }
                }
      ...
      newText: function (text, document, id, stroke) {
        ...
        elmText = document.createElementNS(SVG_NS, 'text')
        elmTextPath = elmText.appendChild(
          document.createElementNS(SVG_NS, 'textPath')
        )
        elmTextPath.href.baseVal = '#' + pathId
        elmTextPath.startOffset.baseVal.newValueSpecifiedUnits(
      ...
      initSvg: function (attachProps, props) {
        ...
        text.styleText.textAnchor = ['start', 'end', 'middle'][
          attachProps.semIndex
        ]
        if (attachProps.semIndex === 2 && !attachProps.lineOffset) {
          // The position never change.
          text.elmOffset.startOffset.baseVal.newValueSpecifiedUnits(
            SVGLength.SVG_LENGTHTYPE_PERCENTAGE,
            50
          )
        }
  ...
  Object.keys(ATTACHMENTS).forEach(function (attachmentName) {
    LeaderLine[attachmentName] = function () {
      return new LeaderLineAttachment(
